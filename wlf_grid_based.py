...
SNAPSHOT_COUNT: int | list[float] = [0.0, 0.5, 1.0, 2.0, 5.0]
SEED = 0
INTERPOLATION_ORDER = {"bimodal": 3, "cauchy": 1}
KDE_BANDWIDTH = {"bimodal": 0.15, "cauchy": 0.25}
...
def main() -> None:
    parser = argparse.ArgumentParser(
        ...
        epilog="Valid pairs: unimodal bimodal, uniform cauchy, corner cauchy.",
    )
    parser.add_argument("--initial", choices=("unimodal", "uniform", "corner"), default="corner")
    parser.add_argument("--target", choices=("bimodal", "cauchy"), default="cauchy")
    args = parser.parse_args()

    experiment_key = (args.initial, args.target)
    if experiment_key not in ALPHA:
        raise ValueError(
            f"unsupported experiment {args.initial!r} -> {args.target!r}; " "valid pairs: unimodal bimodal, uniform cauchy, corner cauchy"
        )
    ...
    augmented_matrix = bmat(
        [[weighted_laplacian, csc_matrix(-np.ones((N_GRID**2, 1)))], [csc_matrix(np.ones((1, N_GRID**2))), csc_matrix((1, 1))]], format="csc"
    )  # On a uniform grid, the quadrature weights m can all be set to 1.
    augmented_solver = factorized(augmented_matrix)

    def solve_weighted_poisson(omega: np.ndarray) -> tuple[np.ndarray, float]:
        solution = augmented_solver(np.concatenate((-omega.ravel(), [0.0])))
        return solution[:-1].reshape(grid.shape), float(solution[-1])
    ...
    snapshots = {0: initial_particles.copy()} if 0 in snapshot_steps else {}
    ...
    for step in range(1, n_steps + 1):
        ...
        # 1. Poisson step
        # ------------------------------------------------------------------
        phi_grid, omega_bar = solve_weighted_poisson(omega_grid)
        velocity_grid = grid.gradient(phi_grid, zero_normal=not grid.periodic)

        # ------------------------------------------------------------------
        # 2. Transport step
        # ------------------------------------------------------------------
        # Preserve omega^n for backtracing and the particle drift.
        ...
        omega_grid = (1.0 - dt) * omega_back + dt * omega_bar
    ...
    save_kl_plot(kl_path, flow_times, flow_kl, target_kde_kl)
    save_snapshot_plot(snapshots_path, snapshots, dt, grid.mesh, target_grid, args.target, target_particles)
