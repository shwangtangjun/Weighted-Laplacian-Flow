...
# Domain and distribution parameters
...
# Target log densities and gradients
...
def grad_log_target_torch(points: torch.Tensor, target: str) -> torch.Tensor:
    """Evaluate the bimodal target score on the particles' device."""
    if target != "bimodal":
        raise ValueError(f"unsupported target {target!r}")
    first_mean, second_mean = bimodal_means(points.shape[-1])
    first_mean = torch.as_tensor(first_mean, dtype=points.dtype, device=points.device)
    second_mean = torch.as_tensor(second_mean, dtype=points.dtype, device=points.device)
    first_delta = points - first_mean
    second_delta = points - second_mean
    first_log = (VON_MISES_KAPPA * torch.cos(first_delta)).sum(dim=-1)
    second_log = (VON_MISES_KAPPA * torch.cos(second_delta)).sum(dim=-1)
    responsibility = torch.sigmoid(first_log - second_log).unsqueeze(-1)
    return -VON_MISES_KAPPA * (responsibility * torch.sin(first_delta) + (1.0 - responsibility) * torch.sin(second_delta))


# Initial and grid densities
...
def uniform_points(count: int, dimension: int, lower: float, width: float, device: torch.device) -> torch.Tensor:
    return lower + width * torch.rand((count, dimension), device=device)


# Torch device and differentiation helpers
...
def scalar_gradient(network: torch.nn.Module, points: np.ndarray | torch.Tensor, device: torch.device, return_numpy: bool = True):
...
# Density estimation and diagnostics
...
def make_snapshot_steps(snapshots: int | list[float], t_final: float, dt: float) -> list[int]:
    """Map an evenly spaced count or explicit times to nearest simulation steps."""
    times = np.linspace(0.0, t_final, snapshots) if isinstance(snapshots, int) else snapshots
    return sorted({int(round(time / dt)) for time in times})
...
def save_snapshot_plot(
    ...
    fig, axes = plt.subplots(
        1,
        len(ordered_steps) + 1,
        figsize=(3.0 * (len(ordered_steps) + 1), 3.0),
        sharex=True,
        sharey=True,
        squeeze=False,
    )

    panels = [(snapshots[step], "C0", f"WLF  t={step * dt:.2f}") for step in ordered_steps]
    panels.append((target_particles, "red", "Ground truth"))
    for column, (particles, color, title) in enumerate(panels):
        ax = axes[0, column]
